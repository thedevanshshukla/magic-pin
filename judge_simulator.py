...
LLM_MODEL = os.getenv("LLM_MODEL") or os.getenv("GROQ_MODEL", "")
...
REPORTS_DIR = Path(__file__).parent / "judge_reports"
...
class ScoreResult:
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {
            ...
            "category_fit_reason": self.category_fit_reason,
            "merchant_fit": self.merchant_fit,
            "merchant_fit_reason": self.merchant_fit_reason,
            "trigger_relevance": self.trigger_relevance,
            "trigger_relevance_reason": self.trigger_relevance_reason,
            "engagement_compulsion": self.engagement_compulsion,
...
Score each dimension 0-10 with clear reasoning. Be STRICT."""
    ...
    def _fallback_score(self, action: Dict) -> ScoreResult:
        ...
        print_fail("LLM scoring failed - fallback detected. Stopping execution.")
        sys.exit(1)
...
class JudgeSimulator:
    def __init__(self, llm: LLMProvider):
        ...
        self.message_reports: List[Dict[str, Any]] = []

    def check_llm_health(self) -> None:
        try:
            print_info("Checking LLM health...")
            response = self.llm.complete("Reply OK", system="You are a helpful assistant. Reply with only the word OK.")
            assert "OK" in response
            print_success("LLM OK")
        except Exception as e:
            print_fail("LLM NOT WORKING — scores invalid")
    ...
    def _score_and_display(self, action: Dict, verbose: bool = True):
        ...
        self.message_reports.append({
            ...
            "scores": {
                ...
                "trigger_relevance": score.trigger_relevance,
            ...
            "reasons": {
                ...
                "trigger_relevance": score.trigger_relevance_reason,
                "engagement_compulsion": score.engagement_reason,
                "penalties": score.penalty_reasons,
            },
            "hint": score.hint,
    ...
    def _final_summary(self):
        ...
        self._write_report(avg, total, pct)

    def _write_report(self, avg: ScoreResult, total: int, pct: float):
        ...
        report = {
            ...
            "message_count": len(self.message_reports),
            "average_scores": {
                "specificity": avg.specificity,
                "category_fit": avg.category_fit,
                "merchant_fit": avg.merchant_fit,
                "trigger_relevance": avg.trigger_relevance,
                "engagement_compulsion": avg.engagement_compulsion,
                "penalties": avg.penalties,
                "total": total,
                "percent": round(pct, 2),
            },
            "messages": self.message_reports,
        }
        dt_str = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
        latest_path = REPORTS_DIR / f"latest_judge_report_{dt_str}.json"
        latest_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
        timestamped_path = REPORTS_DIR / f"judge_report_{dt_str}.json"
        timestamped_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
        print_info(f"Saved report to {latest_path}")
        self.last_report_path = latest_path
...
def run_judge_session(
    ...
    if bot_url:
        os.environ["BOT_URL"] = bot_url

    global TEST_SCENARIO, LLM_PROVIDER, BOT_URL, LLM_API_KEY, LLM_MODEL
    ...
    LLM_API_KEY = os.getenv("LLM_API_KEY") or os.getenv("GROQ_API_KEY", LLM_API_KEY)
    LLM_MODEL = os.getenv("LLM_MODEL") or os.getenv("GROQ_MODEL", LLM_MODEL)

    llm = create_provider()
    judge = JudgeSimulator(llm)
    success = judge.run(TEST_SCENARIO)
    latest_path = getattr(judge, 'last_report_path', REPORTS_DIR / "latest_judge_report.json")
    report = json.loads(latest_path.read_text(encoding="utf-8")) if latest_path.exists() else {}
